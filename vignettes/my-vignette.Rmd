...
vignette: >
  %\VignetteIndexEntry{Using MAPX: step by step}
...
### Loading raw data
Let's first see the procedure with one timepoint (or condition). First, load your data in R and configure it such that it is readable by MAPX commands.
The MAPX pipeline working with melting curve data expects the following column names:
* `protein` – column with protein IDs  
* `description` – protein descriptions, such as those found in FASTA headers  
* `length` – length of the protein in number of amino acids  
* `Ab1` to `AbN` – columns with raw abundance values measured at different temperatures (where `N` is the number of temperatures)
...
```{r tp28_rep1_predictors3}
predictors <- MAPX::cross_join(cordata,feature.predictors,vars=paste0("protein",c(1,2)),mode="full")
```
...
```{r tp28_rep1_training_average_crossmodels}
...
It only makes sense to look at well-defined stable complexes using this approach. This code makes a sub-selection:

```{r complexes.subselection}
...
```{r AI}
data.AIs <- X.AI(data.PCAs$data,sel.cpxs, quality.data, trials=150, weights=data.PCAs$var.explained)
```
