...
#' @param CPs Numeric vector: Complexity parameters to be cross-validated. Default is 0.01.
...
Xmodel.tree.CART <- function(
  ...
  model <- rpart(complex ~ ., data=data, method="class",
                 control=list(cp=CP),parms=list(loss=cost_mat))
